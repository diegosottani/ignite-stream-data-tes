...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      const REDIRECT_URI = makeRedirectUri({useProxy: true}) // helper do Expo e proxy do Expo
      const RESPONSE_TYPE = "token"
      const SCOPE = encodeURI('openid user:read:email user:read:follows')
      const FORCE_VERIFY = true
      const STATE = generateRandom(30) // string aleatória para comparação (segurança)
      ...
      // access_denied é quando, mesmo após success, a pessoa não clica em "autorizar o app..."
  ...
  async function signOut() {
    try {
      ...
      // Remove a autoridade do objeto (está mutando)
  ...
  useEffect(() => {
    // adicionado o valor no header do axios (obrigatório na Twitch)
